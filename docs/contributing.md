...
**必要な環境:**
* Node.js 22 以上（開発は `.nvmrc` の 24 を推奨。CI と同じバージョン）
* npm 10 以上

`nvm` / `fnm` を使っている場合は、リポジトリ直下で `nvm use` / `fnm use` を実行すると
`.nvmrc` のバージョンに切り替わります。CI とローカルでバージョンがずれると、
「CI では通るのにローカルだけ落ちる」たぐいの不一致が起きます。
...
4. 利用者向け画面を追加・改修する前に [UI 文言辞書](./ui-terminology.md) を確認してください
5. 新しい画面用語を導入するときは、実装と同じ変更で UI 文言辞書にも追記してください
...
* **UI 文言:** `npm run lint:wording` で内部状態名や禁止語の混入を確認してください
