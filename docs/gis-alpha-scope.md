...
| 解析結果の平面図重畳 | 一部対応 | 結果画面の正準平面図・模式図では、保存済み水頭の色分け、時刻別アニメーション、凡例、縦断図との同期を利用できる。GIS編集画面のOpenLayers地図への重畳は未対応 |
...
6. **圧力波結果の平面図重畳**（Issue #24、結果画面は実装済み）: 保存済み結果を用いた色分けと縦断図に同期するアニメーションを結果画面に実装した。GIS編集画面のOpenLayers地図への重畳は将来対応とする。
