...
- 設計用水撃圧解析の差分距離は技術書 §8.4.2(2) の一般的な実務目安である50～200 mを基準とする。200 mを超える入力は計算前に不適合とし、50 m未満は計算可能な注意として扱う。
- 管路網の過渡解析と水撃圧対策設備の計算は、同じ物理管路をEPANETのリンクとして定常計算し、管路流量と節点水頭をMOC初期条件へ渡す。EPANETリンクにはMOCの差分格子間隔という概念はなく、両者で一致させるのは物理管路・節点IDと初期状態である。
- 管路ごとに差分距離が異なる場合、共通時間刻みにおける特性線の足を格子間で線形補間する。摩擦項の伝播距離には各管路の `a·Δt` を用い、単に1格子分進んだものとして扱わない。
