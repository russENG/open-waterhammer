...
- **ローカルファースト**: プロジェクトデータはブラウザの IndexedDB、またはローカルの `.owhproj`
  ...
  匿名・クッキーなしのアクセス計測を行う（ページ閲覧のみが対象。詳細は
  [`README.md`](../README.md#データ境界ローカルファースト)）。
...
実装していない**ことの表れである。実務では負圧が生じる区間は吸気弁等の追加対策を要するが、本ツールは
その後続の気液二相の挙動を計算しない（§13、`docs/roadmap.md` の既知の制約も参照）。
...
- **柱分離（キャビテーション・気柱分離）モデルは未実装**。MOC の境界条件は `H = max(CP, 0)` で
  クランプするのみで、蒸気キャビティの形成・収縮・再衝突は計算しない。防護設備を適用したサンプルが
  正当に負圧（hmin < 0）を示すのはこの限界の表れである（§8）。
