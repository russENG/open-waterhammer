...
管路・節点・測点の正本と変換規則は[正準水理モデル](./canonical-hydraulic-model.md) を参照する。測点データの `pipe_id`、`node_id`、`distance_along_pipe` は新規入力での指定を推奨し、列がない旧ファイルも後方互換で読み込む。
...
| `cases` | シナリオ設定 | 境界条件、操作イベント、防護工条件の初期一覧 |
