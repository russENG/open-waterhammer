...
利用者向けのサービス名は、次の表記を使う。

> Open Waterhammer

講演題目は研究成果の名称であり、サービス名として画面のヘッダーやブラウザタイトルには使用しない。全大文字の `OPEN WATERHAMMER` も避ける。ブラウザタイトルでは、サービス内容を示す「農業用パイプライン水撃圧計算」を併記できる。

## 注意・免責

自動評価の近くでは、次の短い確認事項を表示する。

> 自動評価は設計検討を支援する情報です。採用前に、入力条件と計算結果を確認してください。

無保証・責任制限の全文は「このサイトについて」の「ライセンス・無保証」と、単独で流通する計算結果
Excel・計算記録JSONに記載する。ヘッダー、フッター、通常の帳票画面には繰り返し表示しない。
...
- 差分距離（特性曲線法の計算格子間隔。初出では単位 m と実務目安を併記する）
- 初期定常状態（過渡解析の開始前にEPANETで求める管路流量・節点水頭。単に「初期値」とだけ表示しない）
