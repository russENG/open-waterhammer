...
export function ComparePanel({ cases, scenarios, runs }: { cases: Case[]; scenarios: Scenario[]; runs: Run[] }) {
  if (cases.length < 2 || cases.length > 4) return <div className="panel-stack"><div className="panel-title-row"><div><span className="eyebrow">DELTA SHEET / 06</span><h1>比較</h1><p>左の CMP チェックで 2–4 Cases を選択してください。</p></div></div><div className="comparison-empty"><span>2—4</span><p>条件差と Run 結果差を横並びで確認します。</p></div></div>
